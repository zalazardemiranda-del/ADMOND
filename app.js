...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        let localProfilesInit = JSON.parse(localStorage.getItem('rp_local_profiles'));
        if (Array.isArray(localProfilesInit)) {
            let changed = false;
            localProfilesInit.forEach(p => {
                if (p && p.email && p.email.toLowerCase() === 'diego@rodipack.com') {
                    p.password = 'FoxMiranda30';
                    p.rol = 'administrativo';
                    changed = true;
                }
            });
            if (changed) {
                localStorage.setItem('rp_local_profiles', JSON.stringify(localProfilesInit));
            }
        }
    } catch (e) {}
...
window.loadProfilesList = async function() {
    ...
    const defaultProfiles = [
        ...
        { nombre: "Diego Miranda", email: "diego@rodipack.com", rol: "administrativo", departamento: "Administración", password: "FoxMiranda30" }
    ];
    ...
    defaultProfiles.forEach(dp => {
        ...
        if (idx === -1) {
            ...
            if (dp.email.toLowerCase() === 'diego@rodipack.com') {
                ...
                localProfiles[idx].password = 'FoxMiranda30';
            }
        }
    });
...
window.handleSupabaseLogin = async function(event) {
    ...
    const doLocalOrDiegoLogin = async (client) => {
        if (isDiego) {
            const dIdx = localProfiles.findIndex(p => p.email && p.email.toLowerCase() === 'diego@rodipack.com');
            if (dIdx >= 0) {
                localProfiles[dIdx].password = 'FoxMiranda30';
                localProfiles[dIdx].rol = 'administrativo';
            } else {
                localProfiles.push({
                    id: 'usr-diego-miranda',
                    nombre: 'Diego Miranda',
                    email: 'diego@rodipack.com',
                    rol: 'administrativo',
                    departamento: 'Administración',
                    password: 'FoxMiranda30'
                });
            }
            localStorage.setItem('rp_local_profiles', JSON.stringify(localProfiles));
            ...
            if (client) {
                try {
                    client.auth.signUp({
                        email: 'diego@rodipack.com',
                        password: 'FoxMiranda30',
                        options: { data: { nombre: 'Diego Miranda', rol: 'administrativo', departamento: 'Administración' } }
                    }).catch(() => {});
        ...
        const userObj = {
            id: (matchedProfile && matchedProfile.id) || (isDiego ? 'usr-diego-miranda' : `usr-${Date.now()}`),
            email: email,
            nombre: isDiego ? 'Diego Miranda' : ((matchedProfile && matchedProfile.nombre) || email.split('@')[0]),
            rol: isDiego ? 'administrativo' : ((matchedProfile && matchedProfile.rol) || 'colaborador'),
            departamento: isDiego ? 'Administración' : ((matchedProfile && matchedProfile.departamento) || 'Operaciones'),
            password: password
        };
    ...
    if (!window.isSupabaseActive()) {
        if (isDiegoValidPass || (matchedProfile && matchedProfile.password === password)) {
            await doLocalOrDiegoLogin(null);
            return;
        }
        if (errorEl) {
            errorEl.innerText = "Error: Correo o contraseña incorrectos, o configura tu conexión a Supabase.";
    ...
    try {
        ...
        if (error) {
            if (isDiegoValidPass || (matchedProfile && matchedProfile.password === password)) {
                await doLocalOrDiegoLogin(client);
                return;
            }
            ...
            if (errorEl) {
                ...
                errorEl.innerText = `Error: ${userMsg}`;
                errorEl.style.display = "block";
            }
        } else {
            if (data.user) {
        ...
        if (isDiegoValidPass || (matchedProfile && matchedProfile.password === password)) {
            await doLocalOrDiegoLogin(window.SUPABASE_CONFIG ? window.SUPABASE_CONFIG.client : null);
            return;
        }
        if (errorEl) {
...
window.handleGlobalLoginSubmit = async function(event) {
    ...
    try {
        ...
        if (window.isSupabaseActive()) {
            ...
            if (error) {
                // Si es Diego con contraseña válida, autenticar de inmediato y sincronizar en Supabase
                if (isDiegoValidPass) {
                    try {
                        client.auth.signUp({
                            email: 'diego@rodipack.com',
                    ...
                    const userObj = {
                        id: (dIdx >= 0 && localProfiles[dIdx].id) ? localProfiles[dIdx].id : 'usr-diego-miranda',
                        email: 'diego@rodipack.com',
                        nombre: 'Diego Miranda',
                        rol: 'administrativo',
                        departamento: 'Administración',
            ...
            const userObj = {
                id: matchedProfile?.id || (isDiego ? 'usr-diego-miranda' : `usr-${Date.now()}`),
                email: email || 'zalazardemiranda@gmail.com',
                nombre: isDiego ? 'Diego Miranda' : (matchedProfile?.nombre || (isManager ? 'Gerente Principal' : 'Colaborador')),
                rol: isDiego ? 'administrativo' : (matchedProfile?.rol || (isManager ? 'gerente' : 'colaborador')),
                departamento: isDiego ? 'Administración' : (matchedProfile?.departamento || 'Operaciones'),
                password: password || ''
            };
            appState.currentUser = userObj;
            sessionStorage.setItem("rp_session_pass", password);
